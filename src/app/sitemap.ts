...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const isProdDomain =
    process.env.ENV === "PRODUCTION" || process.env.NEXT_PUBLIC_ENV === "PRODUCTION";
  ...
  const moduleSlugs: string[] = await fetch(
    ...
    .then((res) => res.json())
    .then((data) => data?.stories?.map((module: { slug: string }) => module.slug))
    .catch(() => []);
  ...
  const dynamicModuleRoutes: MetadataRoute.Sitemap = moduleSlugs.map((slug) => ({
