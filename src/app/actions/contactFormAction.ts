...
import { getBlocksIdentityToken, recaptchaVarify, sendBlocksEmailService } from "@/lib/utils";
...
export async function contactFormAction(
  ...
  //  API call
  if (token) {
    const recaptchaData = await recaptchaVarify(token, headersList.get("x-forwarded-for") ?? "");
    ...
    const sendEmail = await sendBlocksEmailService({
      ...
      DataContext: {
        ...
        DATENSCHUTZ_VERSION:
          storyDatenschutz.content.body[2].items.at(-1)?.version_number ?? "1.0.0",
        GTC_VERSION: storyGtc.content.body[2].items.at(-1)?.version_number ?? "1.0.0",
      },
    });
