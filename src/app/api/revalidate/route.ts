...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!fullSlug) {
      return NextResponse.json({ message: "No slug found in payload" }, { status: 400 });
    }
    ...
    const normalizedSlug = slugParts
      ...
      .join("/");
    const pathsToRevalidate = [`/de/${normalizedSlug}`, `/en/${normalizedSlug}`];
    ...
    return NextResponse.json({ message: "Error parsing payload", error }, { status: 500 });
  }
}
