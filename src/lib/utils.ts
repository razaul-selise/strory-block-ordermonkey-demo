...
const getConcent = async () => {
  ...
  if (ucData) {
    const consentEntry = Object.entries(ucData ?? {}).find(
      ([, value]) => (value as { name: string; consent: boolean }).name === "reCAPTCHA"
    );
...
export const recaptchaVarify = async (token: string, remoteip: string): Promise<boolean> => {
  try {
    const recaptchaUrl = new URL(process.env.RECAPTCHA_VERIFY_URL ?? "");
    recaptchaUrl.searchParams.append("secret", process.env.RECAPTCHA_SECRET_KEY ?? "");
    ...
    if (!response.ok) {
      console.error("Recaptcha request failed:", response.status, await response.text());
      return false;
    }
